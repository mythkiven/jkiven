...
- (void)setUp{
    ...
    self.label.backgroundColor = RGB_white;
...
- (CAShapeLayer *)shapeLayer{
    ...
    shapeLayer.strokeColor = RGB_gray.CGColor;
    shapeLayer.fillColor = RGB_white.CGColor;
...
- (void)setText:(NSString *)text{
    _text = text;
//    if ([text isEqualToString:@"订单笔数"]) {
    
    if ((text.length == 0)|[text isKindOfClass:[NSNull class]]|!text){
        text = @"0笔";
    }else if (![text hasStr:@"笔"] && ![text isEqualToString:@"订单笔数"]){
        text = [text stringByAppendingString:@"笔"];
    }else {
...
- (void)setTextColor:(UIColor *)textColor{
    ...
    self.label.textColor = RGB_grayNormalText;
    if ([_text isEqualToString:@"订单笔数"]) {
        self.label.textColor = RGB_greenBtn;
    }
}
