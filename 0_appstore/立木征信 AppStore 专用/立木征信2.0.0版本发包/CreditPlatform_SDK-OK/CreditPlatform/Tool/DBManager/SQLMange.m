...
//  CreditPlatform
//
//  Created by ___蒋孝才___ on 16/8/1.
//  Copyright © 2016年 kangcheng. All rights reserved.
//

#import "SQLMange.h"
static SQLMange *tool = nil;
@implementation SQLMange
{
    FMDatabase          *_fmdb;
}
...
+ (instancetype)shareManager {
    static SQLMange *sqlMange = nil;
    
    static dispatch_once_t predicate;
    
    dispatch_once(&predicate, ^{
        if (!sqlMange) {
            sqlMange = [[SQLMange alloc] init];
        }
    });
...
- (instancetype)init {
    if (self = [super init]) {
        ...
        NSString *dbPath = [[NSSearchPathForDirectoriesInDomains(NSDocumentDirectory, NSUserDomainMask, YES) firstObject] stringByAppendingPathComponent:@"sqliteJXC.db"];
