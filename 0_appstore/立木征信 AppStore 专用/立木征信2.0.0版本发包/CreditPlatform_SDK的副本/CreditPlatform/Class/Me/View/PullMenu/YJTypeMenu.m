...
- (void)setMenuType:(YJMenuType)menuType {
    ...
    if (_menuType == YJMenuTypeRecharge) {
        ...
        _contentView.frame =  CGRectMake(0, 0, self.frame.size.width, 335);
...
- (void)layoutSubviews {
    ...
    switch (_menuType) {
        ...
        {
            CGFloat btnW = (SCREEN_WIDTH - 2 * margin) * .5;
            int col = 2;
            //    (count - 1)
            for (int i = 0; i < count; i ++) {
                UIButton *btn = _btnArr[i];
                if (i == 0) {
                    btn.frame = CGRectMake(margin, 0, SCREEN_WIDTH-margin, btnH);
                } else {
                    CGFloat btnX = margin + (i-1)%col * (btnW+margin);
                    CGFloat btnY = (i-1)/col * (btnH) + btnH;
            ...
            _scrollView.contentSize = CGSizeMake(0, _menuH);
            _scrollView.frame =  CGRectMake(0, 0, self.frame.size.width, 335);
