...
- (void)commonInit{
    ...
    _unitColor = RGB_black;
    _yLineNameColor = RGB_black;
    _xLineValueColor = RGB_black;
    _axisLineBackgroundColor = RGB_white;
    _axisColor = RGB_black;
    _isShowSeparate = NO;
    _separateColor = RGB_gray;
...
- (void)setYLineNameLabel{
    if (self.yLineNameArray.count > 0) {
        for (NSInteger i = 0; i < self.yLineNameArray.count; i++) {
            ...
            ZFLabel * label = [[ZFLabel alloc] initWithFrame:CGRectMake(0, 0, width, rect.size.height)];
            label.text = self.yLineNameArray[i];
            
//            label.backgroundColor = RGB_red;
            label.textColor = _yLineNameColor;
            label.textAlignment = NSTextAlignmentRight;
            //字体12
            label.font = [UIFont systemFontOfSize:13.0];
            label.center = label_center;
            if ([self.yLineNameArray[i] isEqualToString:@"订单金额"]) {
                label.font = Font14;
            }
...
- (void)strokePath{
    ...
    for (NSInteger i = 0; i < _xLineSectionCount; i++) {
        if (_isShowSeparate) {
            [self.layer addSublayer:[self xAxisLineSectionShapeLayer:i sectionLength:self.yLineHeight sectionColor:RGB_gray]];
        }else{
