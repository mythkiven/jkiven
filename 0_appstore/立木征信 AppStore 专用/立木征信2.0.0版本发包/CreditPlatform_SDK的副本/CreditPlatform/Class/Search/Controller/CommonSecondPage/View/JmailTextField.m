...
@interface JmailTextField ()<UITableViewDataSource, UITableViewDelegate>
...
- (instancetype)setUpInView:(UIView *)view {
    ...
    self.promptTableView = [[UITableView alloc] initWithFrame:CGRectMake(0, textY+textH+1, SCREEN_WIDTH, pCellH*3) style:UITableViewStylePlain];
    
    self.promptTableView.dataSource = self;
    self.promptTableView.delegate = self;
    self.promptTableView.backgroundColor = RGB_grayBar;
    self.promptTableView.alpha = 0.95;
    [self setUpEmailSuffixArray];
    
    [self.promptTableView registerNib:[UINib nibWithNibName:@"JPromptTableViewCell" bundle:nil]    forCellReuseIdentifier:@"promptCell"];
...
- (void)textFieldDidChanged {
    if ([self.text containsString:@"@"]) {
        ...
        // @后面的subString
        NSString *latterStr = [self.text substringFromIndex:[self.text rangeOfString:@"@"].location+0];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    JPromptTableViewCell *cell = [self.promptTableView dequeueReusableCellWithIdentifier:@"promptCell" forIndexPath:indexPath];
    // @之前的字符串
    NSString *formerStr = [self.text substringToIndex:[self.text rangeOfString:@"@"].location+0];
