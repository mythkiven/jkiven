...
@property (assign,nonatomic) CGFloat  terAlpha;//最后的透明度
...
#warning 显示的警告的背景色
//    wrapperView.backgroundColor = [[UIColor colorWithRed:0/255.0f green:0/255.0f blue:0/255.0f alpha:1] colorWithAlphaComponent:CSToastOpacity];
    ...
    if (ifSucess) {
        messageLabel.textColor=RGB_black;
...
- (void)makeToast:(NSString *)message textColor:(UIColor*)color alpha:(CGFloat)alpha{
    self.terAlpha = alpha;
...
- (CGPoint)centerPointForPosition:(id)point withToast:(UIView *)toast {
    if([point isKindOfClass:[NSString class]]) {
        ...
        if([point caseInsensitiveCompare:@"top"] == NSOrderedSame) {
            ...
            return CGPointMake(self.bounds.size.width / 2, self.bounds.size.height * .45);
        }
    } else if ([point isKindOfClass:[NSValue class]]) {
        return [point CGPointValue];
    }
    
    MYLog(@"Warning: Invalid position for toast.");
