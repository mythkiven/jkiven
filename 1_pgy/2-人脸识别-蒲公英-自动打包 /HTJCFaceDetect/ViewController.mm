...
//  test
//
//  Created by sunjiachen on 15/9/23.
//  Copyright (c) 2015年 sunjiachen. All rights reserved.
//
...
- (void)viewDidLoad {
    ...
    self.navigationController.navigationBar.barTintColor = [UIColor colorWithRed:(0/ 255.0) green:(133 / 255.0) blue:(170 / 255.0) alpha:1];
...
-(void)setDetect {
    ...
    NSMutableArray *detectTypes = [NSMutableArray arrayWithArray:@[@0, @1, @2, @3, @4, @1]];
...
//    BOOL isRandom = [[detectTypes lastObject] boolValue];
    ...
    if (!_appDelegate.liveDetectTypes || _appDelegate.liveDetectTypes.count == 0) {
        detectTypes = [self random:detectTypes withRandomNum:3];
    }
    
    
    [manager navigationType:0];
    [manager isNeedRandom:NO];//是否随机
    [manager liveDetectTypeArray:@[@0, @1, @2, @3, @4]];//检测项目
