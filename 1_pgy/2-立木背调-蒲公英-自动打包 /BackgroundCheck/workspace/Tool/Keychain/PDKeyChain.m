...
static NSString * const kPDDictionaryKey = @"com.YJ.dictionaryKey";
static NSString * const kPDKeyChainKey = @"com.YJ.keychainKey";

@implementation PDKeyChain

+ (NSString *)loadUniqueIDInKeyChain{
    NSString *ID = [self keyChainLoad];
    if ([ID isKindOfClass:[NSString class]] && ID){
        if (ID.length) {
            return ID;
        }
    }
    ...
    if (uuid) {
        ...
        NSString * NEW_ID = [NSString stringWithFormat:@"IOS%@",str];
        [self keyChainSave:NEW_ID];
        return NEW_ID;
    }
