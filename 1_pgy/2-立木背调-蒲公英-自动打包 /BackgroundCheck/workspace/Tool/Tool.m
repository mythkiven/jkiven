...
/** 6-16位 
 ...
 0:字母数字下划线两两组合
 */
+ (NSInteger)verifyPassword:(NSString *)passWord {
    
    NSString *numRegex = @"^[0-9]{6,16}$";
    NSPredicate *numRegexP = [NSPredicate predicateWithFormat:@"SELF MATCHES %@",numRegex];
    if ([numRegexP evaluateWithObject:passWord]) {
        return 1;
    }
    NSString *letterRegex = @"^[A-Za-z]{6,16}$";
    NSPredicate *letterRegexP = [NSPredicate predicateWithFormat:@"SELF MATCHES %@",letterRegex];
    if ([letterRegexP evaluateWithObject:passWord]) {
        return 2;
    }
    NSString *_Regex = @"^_{6,16}$";
    NSPredicate *_RegexP = [NSPredicate predicateWithFormat:@"SELF MATCHES %@",_Regex];
    if ([_RegexP evaluateWithObject:passWord]) {
        return 3;
    }
    NSString *PunctRegex = @"^[^\w]*$";
    NSPredicate *PunctRegexP = [NSPredicate predicateWithFormat:@"SELF MATCHES %@",PunctRegex];
    if ([PunctRegexP evaluateWithObject:passWord]) {
        return 4;
    } 
    return 999;
