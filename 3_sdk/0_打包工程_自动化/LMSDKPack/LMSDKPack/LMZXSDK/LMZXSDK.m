...
// 预留方案,如果客户调用SDK,未能成功调用起SDK, 就使用此预留方案.
// IOS11 兼容性方案.
@property (nonatomic,strong)  UIViewController *originalVC;
...
-(LMZXSDK *)initWithApikey:(NSString *)lmApikey uid:(NSString *)lmUid callBackUrl:(NSString *)callBackUrl{
    ...
    // 没有提前设置,就默认生产的,如果外界提前设置,就使用外界设置的.
    if(!_lmzxTestURL){
    ...
    // 预传信息
    _lmzxAbleEditPreMobileNum =YES; // 默认可以修改手机号
    _lmzxPreMobileNum =nil; // 默认 nil
...
-(void)startFunction:(LMZXSDKFunction)function authCallBack:(LMZXAuthBlock)lmzxAuthBlock{
    ...
    // 提示用户传入参数格式有问题啊
    if (![_lmzxUid isKindOfClass:[NSString class]]) {
        LMLog(@"立木征信 SDK: 请确认传入的lmzxUid是否为NSString格式");
        return;
    }
    if (![_lmzxApiKey isKindOfClass:[NSString class]]) {
        LMLog(@"立木征信 SDK: 请确认传入的lmzxApiKey是否为NSString格式");
        return;
    }
    if (![_lmzxCallBackUrl isKindOfClass:[NSString class]]) {
        LMLog(@"立木征信 SDK: 请确认传入的lmzxCallBackUrl是否为NSString格式");
        return;
    }
    
    [LMZXTool setObject:@"" forKey:lmzx_app_choose_channel];
...
//#warning  TEST 内部使用
    ...
    if (_channel&&_channel.length) {
        [LMZXTool setObject:_channel forKey:lmzx_app_choose_channel];
    }
    ...
    switch (function) {
        ...
        case LMZXSDKFunctionMobileCarrie:{
            ...
            if (self.lmzxPreMobileNum) {
                LMLog(@"立木征信Log: 预传运营商手机号:%@",self.lmzxPreMobileNum);
                if(self.lmzxAbleEditPreMobileNum) {
                    LMLog(@"立木征信Log: 可修改预传手机号");
                }else{
                    LMLog(@"立木征信Log: 不可修改手机号");
                }
            }
...
#pragma mark - IOS11预留方案
-(void)lmzxSetSourceViewController:(UIViewController*)viewController{
    if(viewController){
