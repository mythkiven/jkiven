 ...
 */
typedef void(^SearchSuccess)(id obj,NSDictionary * dic);

/** 失败回调:
 @param error 查询失败的 msg,展示在 SDK 中,替代 block: SearchFailure
 @param type  验证码状态,输出给客户(枚举)
 @param info  含有任务 id,等其他信息{@"taskID":@"任务 ID",...}
 */
typedef void(^SearchFailure)(NSString * error, NSInteger type, NSDictionary* info);
 ...
 */
typedef void(^ LoginStatus)(NSInteger status, NSString *token);
...
/**
 登录状态回调 0,成功
 */
@property (nonatomic, copy) LoginStatus  loginStatus;
