...
//    [YJShortLoadingView yj_makeToastActivityInView:self.view];
    ...
    NSDictionary *dicParams =@{@"method" : @"api.mobile.area",
...
//        [YJShortLoadingView yj_hideToastActivityInView:self.view];
        ...
        if (!obj | !obj4) {
            ...
            NSDictionary *list = [obj objectForKey:@"data"];
            NSString *code;
            ...
            if (![list isKindOfClass:[NSNull class]]) {
                if ([code isEqualToString:@"0000"]) {//有数
                    ...
                    if ([code isEqualToString:@"1013"]) {
                        [sself.view makeToast:@"请输入正确的手机号"];
                    }else{
...
//    __block typeof(self) sself = self;
    ...
    LMZXMobilCarrieLoadingAnimationVC  * loadingVC = [[LMZXMobilCarrieLoadingAnimationVC alloc] init];
...
//    LMLog(@"格式校验：first:%@-twO:%@-three:%@",self.firstTextField,self.twoTextField,self.threeTextField);
    ...
    if (self.verifyNum == 2) {
        if (!self.firstTextField.length) {
            [self.view makeToast:@"请输入手机号码" ifSucess:NO];
            return NO;
    ...
    if (self.firstTextField.length) {
        if(![NSString isMobileNumber:self.firstTextField]){
            [self.view makeToast:@"请输入正确的手机号码" ifSucess:NO];
            return NO;
        }
