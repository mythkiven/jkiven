...
-(void)drawRect:(CGRect)rect{
    ...
    _topLine.hidden = NO;
    _bottomLine.hidden = NO;
    if (_searchItemType == SearchItemTypeCreditCardBill && _model.location == 1) {
...
-(void)layoutSubviews{
    ...
    _textField.frame = CGRectMake(15+CGRectGetMaxX(_Name.frame), 0, width-CGRectGetMaxX(_Name.frame)-45, height);
...
//            }
    ...
    // 重新布局
    if (model.searchItemType == SearchItemTypeCreditCardBill && _model.location == 1) {
        _creditLable.hidden =NO;
    }else{
        _creditLable.hidden =YES;
    }
...
-(void)setCreditMail:(NSString *)creditMail{
    ...
    // 重新布局
    if (_model.searchItemType == SearchItemTypeCreditCardBill && _model.location == 1) {
