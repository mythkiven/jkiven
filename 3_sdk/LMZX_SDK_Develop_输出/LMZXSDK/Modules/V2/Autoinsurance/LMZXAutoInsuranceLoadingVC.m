...
@interface LMZXAutoInsuranceLoadingVC ()

@end

@implementation LMZXAutoInsuranceLoadingVC
...
- (void)loadAutoInsurance {
                    ...
                    dispatch_async(dispatch_get_main_queue(), ^{
                        [[[[UIApplication sharedApplication] keyWindow] rootViewController] dismissViewControllerAnimated:YES completion:^{
                            ...
                            if ([LMZXSDK shared].lmzxResultBlock) {
                                [LMZXSDK shared].lmzxResultBlock(2,LMZXSDKFunctionAutoinsurance, nil, token);
                            }
                        }];
                    });
    ...
    [self.lmzxBaseSearchDataTool searchCarInsuranceDataWithQueryInfo:self.lmQueryInfoModel searchSuccess:^(id obj, NSDictionary *dic) {
        ...
        [sself.controlView successAnimation:^{
            ...
            if ([LMZXSDK shared].lmzxQuitOnSuccess==YES) {
                ...
                dispatch_async(dispatch_get_main_queue(), ^{
                    ...
                    [[[[UIApplication sharedApplication] keyWindow] rootViewController] dismissViewControllerAnimated:YES completion:^{
                        if ([LMZXSDK shared].lmzxResultBlock) {
                            [LMZXSDK shared].lmzxResultBlock(0,LMZXSDKFunctionAutoinsurance, obj, dic[@"taskID"]);
                        }
                    }];
        ...
        if ([LMZXSDK shared].lmzxResultBlock) {
            [LMZXSDK shared].lmzxResultBlock(code,LMZXSDKFunctionAutoinsurance, @"nil", taskID);
        }
