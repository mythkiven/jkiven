...
@interface LMZXCentralBankLoadingVC ()

@end

@implementation LMZXCentralBankLoadingVC
...
- (void)loadCentralBank {
                    ...
                    dispatch_async(dispatch_get_main_queue(), ^{
                        [[[[UIApplication sharedApplication] keyWindow] rootViewController] dismissViewControllerAnimated:YES completion:^{
                            ...
                            if ([LMZXSDK shared].lmzxResultBlock) {
                                [LMZXSDK shared].lmzxResultBlock(2,LMZXSDKFunctionCentralBank, nil, token);
                            }
                        }];
                    });
                }];
    ...
    [self.lmzxBaseSearchDataTool searchCentralBankDataWithQueryInfo:self.lmQueryInfoModel searchSuccess:^(id obj, NSDictionary *dic) {
        ...
        [sself.controlView successAnimation:^{
            ...
            if ([LMZXSDK shared].lmzxQuitOnSuccess==YES) {
                ...
                dispatch_async(dispatch_get_main_queue(), ^{
                    ...
                    [[[[UIApplication sharedApplication] keyWindow] rootViewController] dismissViewControllerAnimated:YES completion:^{
                        if ([LMZXSDK shared].lmzxResultBlock) {
                            [LMZXSDK shared].lmzxResultBlock(0,LMZXSDKFunctionCentralBank, obj, dic[@"taskID"]);
                        }
                    }];
        ...
        if ([LMZXSDK shared].lmzxResultBlock) {
            [LMZXSDK shared].lmzxResultBlock(code,LMZXSDKFunctionCentralBank, @"nil", taskID);
        }
