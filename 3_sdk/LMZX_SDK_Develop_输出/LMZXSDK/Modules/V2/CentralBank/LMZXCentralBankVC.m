...
// 自定一个
#import "LMZXCentralBankLoadingVC.h"

// 键盘延时弹出时间:有些特殊键盘加载时间较长,如果是页面没有完全弹出之前,加载键盘,会卡顿.
#define LM_TextfiledPopDelay 0.5

@interface LMZXCentralBankVC ()<UITextFieldDelegate,UIGestureRecognizerDelegate>
{
...
@implementation LMZXCentralBankVC
...
//    LMLog(@"first:%@-two:%@-three:%@",self.firstTextField,self.twoTextField,self.threeTextField);
    ...
    self.lmQueryInfoModel.middleAuthCode = self.threeTextField;
    
    LMZXCentralBankLoadingVC *loadingVc = [[LMZXCentralBankLoadingVC alloc] init];
...
-(BOOL)verificateData {
    ...
    if (!self.firstTextField.length) {
        ...
        return NO;
    } else if (!self.self.threeTextField.length) {
        [self.view makeToast:@"请输入身份验证码" ifSucess:NO];
        return NO;
    }
