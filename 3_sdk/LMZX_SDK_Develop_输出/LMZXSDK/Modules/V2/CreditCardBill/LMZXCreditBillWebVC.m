...
-(void)back{
    ...
    // 139 连续加载两次
    if ([self.webView.request.URL.absoluteString isEqualToString:@"http://html5.mail.10086.cn/"]) {
...
//#warning TEST-TEST
    ...
    // 轮训动画页面:
    LMZXCreditBillLoadingVC  * loadingVC = [[LMZXCreditBillLoadingVC alloc] init];
    loadingVC.lmQueryInfoModel = model;
    loadingVC.type = self.type;
    [self.navigationController pushViewController:loadingVC animated:YES];
    
}
#pragma mark NSHTTPCookieStorage 读取 cookie
