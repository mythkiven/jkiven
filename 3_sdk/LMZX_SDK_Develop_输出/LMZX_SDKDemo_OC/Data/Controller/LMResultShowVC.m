...
- (void)viewWillAppear:(BOOL)animated {
    ...
    if ([LMZXSDK shared].lmzxQuitOnSuccess==NO) {
        ...
        self.resultView.text = @"轮训中,请稍等..";
        if (!_timer ) {
            [self startRequestData];
            [self performSelector:@selector(startCycleRequest) withObject:nil afterDelay:5.0];
            
        }
        
    }else {
        self.resultView.text = @"查询结果,请稍等..";
        [self queryResultWithFunction:_function taskId:_taskId];
    }
...
- (void)startCycleRequest {
    // 添加定时器前，是否已经终止了查询
    if (!_timer) {
        _timer = [NSTimer timerWithTimeInterval:1.0 target:self selector:@selector(startRequestData) userInfo:nil repeats:YES];
...
- (void)startRequestData {
    ...
    [self post:lm_url params:dict success:^(id responseObj) {
        NSLog(@"获取结果状态-------%@",responseObj);
        
        if ([responseObj[@"code"] isEqualToString:@"0000"]) { // 1 查询成功
            [sself queryResultWithFunction:_function taskId:_taskId];
            [_timer invalidate];
            _timer = nil;
            sself.resultView.text = @"结果查询成功,开始加载数据..";
...
#pragma mark- **3.查询结果**
- (void)queryResultWithFunction:(LMZXSDKFunction)function taskId:(NSString*)taskID {
    ...
    NSLog(@"%@",dic);
    [self post:lm_url params:dic success:^(id obj) {
        ...
        if (obj) {
            ...
            weakSelf.resultView.text = [[@"token:\n" stringByAppendingString:taskID] stringByAppendingString:[NSString stringWithFormat: @"\n\n 结果:\n%@", txt]];
        } else{
            weakSelf.resultView.text = [[@"token:\n\n"stringByAppendingString:taskID]  stringByAppendingString: @"\n\n 结果:无数据"];
        }
