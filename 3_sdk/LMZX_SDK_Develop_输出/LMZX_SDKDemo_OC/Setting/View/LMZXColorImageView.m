...
@implementation LMZXColorImageView
...
- (void)touchesBegan:(NSSet<UITouch *> *)touches withEvent:(nullable UIEvent *)event {
    ...
    if (pow(pointL.x - self.bounds.size.width/2, 2)+pow(pointL.y-self.bounds.size.width/2, 2) <= pow(self.bounds.size.width/2, 2)) {
        ...
        NSDictionary *color = [self colorAtPixel:pointL];
        if (self.currentColorBlock1) {
            
            self.currentColorBlock1(color);
        }
    }
}
...
//        }
        ...
        if (self.currentColorBlock1) {
            ...
            self.currentColorBlock1([self colorAtPixel:pointL]);
        }
    }
}
...
//获取图片某一点的颜色
- (NSDictionary *)colorAtPixel:(CGPoint)point {
...
//获取图片某一点的颜色
- (UIColor *)colorAtPixel1:(CGPoint)point {
    if (!CGRectContainsPoint(CGRectMake(0.0f, 0.0f, self.image.size.width, self.image.size.height), point)) {
        return nil;
    }
