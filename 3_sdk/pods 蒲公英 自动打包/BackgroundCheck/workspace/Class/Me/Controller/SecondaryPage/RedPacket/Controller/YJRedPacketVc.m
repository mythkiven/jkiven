...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (cell == nil) {
        ...
        cell.detailTextLabel.textColor = RGB_grayNormalText;
        UIView *separateLineDown = [self separateLine];
        separateLineDown.frame= CGRectMake(0, 0, SCREEN_WIDTH, 0.5);
        [cell.contentView addSubview:separateLineDown];
        
        cell.imageView.image = [UIImage imageNamed:@"icon_redPacket_small"];
        
    }
    
    YJRechargeHistoryListModel *model = self.redPacketArr[indexPath.row];
//    YJRedPacketModel *model = self.redPacketArr[indexPath.row];
    
    cell.textLabel.attributedText = [self redPacketAmount:[NSString stringWithFormat:@"%@元",model.rechangeAmt]];
    
    cell.detailTextLabel.text = [NSString stringWithFormat:@"%@ 获得",[model.rechangeDateStr substringFromIndex:5]];
