...
- (void)setSearchItemType:(LMZXSearchItemType)searchItemType {
    ...
    switch (searchItemType) {
        ...
        case LMZXSearchItemTypeNetBankBill:{//网银
            _firstTime = 7;_circleTime = 3;_timeOut = 300;
            _bizType = lmzx_kBizType_ebank;
            break;
            
        }case LMZXSearchItemTypeMaimai:{//脉脉
            _firstTime = 0;_circleTime = 2;_timeOut = 300;
            _bizType = lmzx_kBizType_maimai;
            break;
            
        }case LMZXSearchItemTypeLinkedin:{//领英
            _firstTime = 0;_circleTime = 2;_timeOut = 300;
...
//            
//        }
        case LMZXSearchItemTypeDiDiTaxi:{//滴滴
            _firstTime = 0;_circleTime = 2;_timeOut = 300;
...
- (void)stopSearch {
    // 如果正在查询
    if (_isSearch) {
        [self handleFailure:@"用户取消查询" action:LMZXSDKSearchActionCancel  dic:@{@"token":_token}];
        [self removeTimer];
...
- (void)removeTimerWithTimeOut {
    [self removeTimer];
    [self handleFailure:@"查询超时，请稍后重试。" action:LMZXSDKSearchActionOtherError  dic:@{@"token":_token}];
    LMLog(@"立木征信Log:请求超时");
}
...
-(void)reBeginLoading:(NSNotification*)noti{
    ...
    if ([sign isKindOfClass:[NSString class]] && sign.length) {
        ...
        if (params.allKeys.count<1) {
            [weakSelf handleFailure:@"签名异常" action:LMZXSDKSearchActionOtherError  dic:@{@"token":_token}];
            return;
        }
      ...
      //  LMLog(@"立木征信Log:客户完成加签,建立查询任务中");
        [LMZXHTTPTool post:LMZXSDK_url params:params success:^(id obj) {
            BOOL obj4 =[obj isKindOfClass:[NSDictionary class]];
            if (!obj | !obj4) {
                [weakSelf handleFailure:@"查询失败" action:LMZXSDKSearchActionOtherError  dic:@{@"token":_token}];
                LMLog(@"立木征信Log:建任务失败:%@",obj);
            }else if ([obj[@"code"] isEqualToString:@"0010"]) {
                _token = obj[@"token"];
                LMLog(@"立木征信Log:受理成功,token:%@",_token);
                [weakSelf addTimer];
                
                // 回调客户:
                [LMZXSDK shared].lmzxResultBlock(1,[LMZXSDK shared].lmzxFunction, @"受理成功", _token);
            ...
            LMLog(@"立木征信Log:建任务失败:%@",error);
        }];
        
    }else{
        [self handleFailure:@"签名异常" action:LMZXSDKSearchActionOtherError  dic:@{@"token":_token}];
    }
...
//        else
       ...
       if (self.queryInfoModel.checkTypeForSMS == LMZXCommonSendMsgTypePhone ) {
            _jPopTextFiledView.sendMsgType = self.queryInfoModel.checkTypeForSMS;
        ...
        _jPopTextFiledView.CancleBlock=^(){
            //错误类型 根据 code 决定
            [sself handleFailure:@"用户取消查询" action:LMZXSDKSearchActionCancel  dic:@{@"token":__token}] ;
