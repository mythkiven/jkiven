...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    switch (self.searchItemType) {
        case  LMZXSearchItemTypeCarSafe:{// 车险
            ...
            break;
        }case LMZXSearchItemTypeNetBankBill:{// 网银流水
            LMZXListHookModel *md = self.listData[indexPath.row];
    ...
    cell.tapListHookCell =^(LMZXListHookModel *model){
        ...
        if (sself.searchItemType == LMZXSearchItemTypeCarSafe | sself.searchItemType == LMZXSearchItemTypeNetBankBill) {// 车险 //网银
            if (sself.selectedOneCity) {
                sself.selectedOneCity(model);
            }
            [sself jPopSelfWith:0.3];
            
        }else if (sself.searchItemType == LMZXSearchItemTypeCreditCardBill){// 信用卡
