...
/*! @brief 3-> 预传参数调用 SDK 功能. 必须传入全部的参数,直接进入动画页面.
 *  @param function 启动的某个功能
 *  @param paramDic 预传参数. 必须传入查询所需的全部参数, 则直接进入动画页面.
 *  @param lmzxAuthBlock 授权回调
 */
...
@"otherInfo":""   // 非必传,特殊城市例外: 北京移动:传入客服密码,广西电信:传入身份证号码,山西电信:传入身份证号码.
@"identityCardNo":@"身份证号", // 非必传
@"identityName":@"真实姓名",  // 非必传
