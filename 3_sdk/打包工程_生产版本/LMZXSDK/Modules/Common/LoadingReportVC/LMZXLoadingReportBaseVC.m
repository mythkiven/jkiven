...
-(void)viewWillAppear:(BOOL)animated{
    ...
    /**
     ...
     // 目前仅仅运营商传递参数
     if ([LMZXSDK shared].lmzxFunction == LMZXSDKFunctionMobileCarrie&&([self isBeingPresented] || [self isMovingToParentViewController])) {
     _isPresent =YES;
     self.navigationItem.leftBarButtonItem = [UIBarButtonItem backBarButtonItemtarget:self action:@selector(backOut)];
     } else {
     // pop to here
     }
     */
...
//        [LMZXSDK shared].lmzxResultBlock(-6,[LMZXSDK shared].lmzxFunction, @"", @"");
//    }
...
- (void)handleSuccessWithResponse:(id)obj info:(NSDictionary *)dic {
    ...
    [self.controlView successAnimation:^{
        //页面跳转
        // 注意:
        // 注意:self dismissViewControllerAnimated
        // 注意:
...
- (void)handleFailureWithError:(NSString *)error code:(NSInteger)code info:(NSDictionary *)dic {
    ...
    if ([LMZXSDK shared].lmzxQuitOnFail ) {
       // [[[[UIApplication sharedApplication] keyWindow] rootViewController] dismissViewControllerAnimated:YES completion:nil];
        if (_isPresent) {
            [self performSelector:@selector(outSelfWith) withObject:nil afterDelay:1.0];
        } else {
            [self jOutSelf];
        }
