 ...
 ******************演示结果处理****************
 APP应该请求商户的服务器
 1.若SDK设置为登录成功退出，先从商户服务器获取查询状态，然后再获取查询结果
 2.若SDK设置为查询成功退出，直接从商户服务器获取查询结果
 
 注：本示例是请求立木服务器，模拟结果数据处理
 */
...
- (void)viewDidLoad {
    ...
    if ([LMZXSDK shared].lmzxQuitOnSuccess==NO) {
        ...
        self.resultView.text = @"请从贵司服务器获取结果数据..";
...
- (void)queryResult {
    ...
    [self post:[lm_url stringByAppendingString:@"/api/gateway"] params:dic success:^(id obj) {
        ...
        if (obj) {
            ...
            weakSelf.resultView.text = [[@"token:\n" stringByAppendingString:_token] stringByAppendingString:[NSString stringWithFormat: @"\n\n 结果:\n%@", txt]];
        } else{
            weakSelf.resultView.text = [[@"token:\n\n"stringByAppendingString:_token]  stringByAppendingString: @"\n\n 结果:无数据"];
        }
